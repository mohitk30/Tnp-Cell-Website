...
//$query="SELECT *,drdate-CURDATE()  AS Datediff from drives where isapproved= 'yes' and drdate >= CURDATE() order by Datediff ";

$query="SELECT *,DATEDIFF(drdate, CURDATE())  AS Datediff from drives where isapproved= 'yes' and drdate >= CURDATE() order by Datediff ";
 $result_setdrives=mysqli_query($dbs,$query);
  $countupd=mysqli_num_rows($result_setdrives);
...
<body>
  ...
  <div class="container-fluid nav-in-company">
    <span class="navbar-text fs-4 ">
      <a href="/" class="text-decoration-underline"> Home</a>> Upcoming Placement Drives  
    </span>
  </div>
</nav>
<section>
...
$redclass="red-col-text";
  ...
  <div class="card-body ">
    ...
    <a href="javascript:void(0);" class="card-link">Min CGPA: <?=$drives['orgmcgpa']?> </a>
  </div>
</div>
    <?php }?>
