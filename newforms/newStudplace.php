...
<head>
    ...
    <title>New Student placement info  Form
</title>
</head>
...
//echo '<script>alert("Done")</script>';
         $dbadstud=db_connect();
          ...
          //  $semal=$_POST['semail'];
           ...
           $query="INSERT INTO sp_info(spenroll,spspplacecomp,spplaceyear,spplacectc)  VALUES('".$senroll."','".$pinc."','".$pyer."','".$pctc."')";
           $result_setadddrive=mysqli_query($dbadstud,$query);

           if($result_setadddrive){
            echo '<script>alert("Placement  Data Inserted")</script>';
            header('refresh:3');
           }else{
            echo '<script>alert("Placement  Data Not Inserted.Try Again After Sometime")</script>';
           }
  ...
  <div class="container-fluid nav-in-company">
    <span class="navbar-text fs-4 ">
      <!-- <a href="<?= $admlink ?>" class="text-decoration-underline"> Back</a>>   -->  New Student Placement info
    </span>
  </div>
</nav>
<section>
    ...
    <div class="input-group mb-3 mt-4">
      ...
      <div class="input-group mb-3 mt-4 w-50">
         ...
         <select class="form-select" aria-label="Default select example" name="scyear" required>
          ...
          <input type="text" class="form-control" min="1" name="pin" placeholder="Company name" aria-label="Server">   
    </div> 
       <div class="input-group mb-3 mt-4">
         <span class="input-group-text">Placed Year</span>
          <input type="number" class="form-control" min="2012" name="pyr" placeholder="Year of placement" aria-label="Username">
          <span class=" mx-2"></span>
          <span class="input-group-text">CTC</span>
          <input type="number" class="form-control" min="1" name="pctc" placeholder="CTC" aria-label="Server">
    </div>   
