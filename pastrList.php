...
<!-- <a class="text-decoration-underline" href="<?=  $admlink ?>">Back </a> --> Here is some past recuriters
    </span>
  </div>
</nav>

<section>
<div class="px-5 pt-2 mt-5 mx-2 fromdirectorheadinf my-auto"> Past Recruiters</div>
...
?>
  <tbody>
      <?php
       while($coms=mysqli_fetch_assoc($result_sets))
       { ?>
    <tr >
      <th scope="row" class="px-5">   <?php echo  $coms['pstrname'] ?></th>
      <td> <?php echo  $coms['pstrlink'] ?></td>
      <td> <a  href="/cruds/updatepastr.php?pastrid=<?php echo $coms['pstrid'];?>"><button class="btn btn-primary"> Update</button></a></td>
      <td>  <button onclick="deletepastrid(<?php echo $coms['pstrid'];?>)" class="btn btn-danger">Delete</button > </td>
